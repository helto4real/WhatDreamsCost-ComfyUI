...
FOLDERS_FILE = CONFIG_DIR / "timeline_image_folders.json"
THUMB_CACHE_DIR = NODE_DIR / "thumbnail_cache"
IMAGE_EXTENSIONS = {".png", ".jpg", ".jpeg", ".webp", ".bmp"}


@dataclass(frozen=True)
class TimelineImageFolder:
    alias: str
    path: str
    enabled: bool = True
...
def default_folder():
    return TimelineImageFolder(alias="input", path=os.path.normpath(folder_paths.get_input_directory()), enabled=True)
...
def load_folders():
    ...
    for entry in data.get("folders", []):
        ...
        if alias in seen or not path:
            continue
        folders.append(TimelineImageFolder(alias=alias, path=path, enabled=bool(entry.get("enabled", True))))
        seen.add(alias)
...
def add_folder(alias, path):
    ...
    folders.append(TimelineImageFolder(alias=alias, path=path, enabled=True))
