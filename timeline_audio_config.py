...
FOLDERS_FILE = CONFIG_DIR / "timeline_audio_folders.json"
AUDIO_EXTENSIONS = {".wav", ".mp3", ".flac", ".ogg", ".m4a", ".aac", ".aiff", ".aif", ".opus"}


@dataclass(frozen=True)
class TimelineAudioFolder:
    alias: str
    path: str
    enabled: bool = True
...
def default_folder():
    return TimelineAudioFolder(alias="input", path=os.path.normpath(folder_paths.get_input_directory()), enabled=True)
...
def load_folders():
    ...
    for entry in data.get("folders", []):
        ...
        if alias in seen or not path:
            continue
        folders.append(TimelineAudioFolder(alias=alias, path=path, enabled=bool(entry.get("enabled", True))))
        seen.add(alias)
...
def resolve_audio_path(alias, filename):
    ...
    if not candidate.is_file():
        raise FileNotFoundError(f"Audio not found: {alias}/{filename}")
    return candidate
...
def add_folder(alias, path):
    ...
    folders.append(TimelineAudioFolder(alias=alias, path=path, enabled=True))
