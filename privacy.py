"""Local workflow encryption helpers for LTX Director."""
...
def decrypt_state(payload: Any, base_dir: str | os.PathLike[str] | None = None) -> Dict[str, Any]:
    if isinstance(payload, str):
        try:
            ...
            raise PrivacyError(f"Encrypted LTX Director data is not valid JSON: {exc}") from exc
    if not is_encrypted_payload(payload):
        raise PrivacyError("LTX Director data is not an encrypted privacy payload.")
    key, key_id = _load_or_create_key(base_dir, create=False)
    payload_key_id = str(payload.get("keyId", ""))
    if payload_key_id != key_id:
        raise PrivacyError("Encrypted LTX Director data was created with a different local privacy key.")
    try:
        ...
        raise PrivacyError(f"Could not decrypt LTX Director data: {exc}") from exc
    if not isinstance(loaded, Mapping):
        raise PrivacyError("Encrypted LTX Director data did not contain a state object.")
    return dict(loaded)
